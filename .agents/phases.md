...
- [ ] Reset from zero twice, run advisors/tests, and regenerate types. **Partially blocked, honestly recorded, not faked:** `supabase gen types` and `supabase functions serve` both appeared to require Docker/Podman to run a `postgres-meta` container **even when targeting a remote database via `--db-url`** at the time this was written. `supabase db reset` was **not** attempted at all: it drops and rebuilds the entire target database, which is a meaningfully more destructive action against a live hosted project than the phase's original "local disposable stack" framing envisioned, so it was deliberately not run without a separate explicit confirmation. `frontend/src/lib/database.types.generated.ts` was created, found to (apparently) require Docker, and deleted rather than left as a broken artifact; `database.types.ts` remained a hand-written placeholder through Phase 11.
  **Correction, 2026-07-28**: the "`gen types` needs Docker" belief was wrong for CLI
  2.110.0 — caught by a Phase 13 documentation walkthrough that actually ran the
  command instead of trusting this note. `supabase gen types typescript --linked`
  works with no container runtime at all. `database.types.ts` has since been
  regenerated for real (see Phase 11's entry below) — `supabase functions serve` still
  genuinely requires Docker, that part of this note stands.
...
- [ ] **Verification checkpoint:** Bootstrap known-email test accounts out of band, demonstrate Officer/Supervisor and disabled cases, and complete password recovery without any Admin UI/account. **Partially satisfied**: Officer/Supervisor/disabled-account behavior was demonstrated live against real (throwaway) accounts, not a description. Password recovery was **not** completed end-to-end — that needed a real deliverable email address and working SMTP, neither available at the time.
  **Progress 2026-07-28**: SMTP is now configured (Gmail via an app password —
  `config.toml`'s `[auth.email.smtp]`), removing half of that blocker. A real
  `inviteUserByEmail` call against a real inbox (`doorvi74@gmail.com`, supplied by the
  project owner specifically for this test) succeeded at the API level with no error.
  **Actual inbox delivery is not yet confirmed** — see `supabase/scripts/README.md`'s
  `bootstrap-user.mjs` section, updated once that's checked. Full password-recovery
  completion (an actual password reset through the real flow, not just an invite send)
  still needs a real roster run, which remains a separate, deferred decision.
...
- [x] Create stable TanStack Query keys and typed row-to-UI mappers.
  ...
  live `information_schema` dump; `supabase gen types` believed blocked on Docker/Podman
  per Phase 9). `src/lib/supabase/pagination.ts` adds `PagedResult<T>`, `clampPageSize`,
  `toRange`, `withTieBreaker`.
  **Superseded 2026-07-28**: `database.types.ts` is now real `supabase gen types`
  output (the Docker belief was wrong — see Phase 9's corrected entry and Phase 13's
  documentation walkthrough). Regenerating surfaced one real thing the hand-written
  version got wrong: Postgres's view-column introspection reports every column of a
  view as nullable, including `machines_with_derived`'s base-machine columns (joined
  through a `NOT NULL` foreign key that can never actually be null) and the four
  summary views' aggregate counts (their own SQL already `COALESCE`s these to 0).
  `mappers.ts` now handles the former with a documented `assumeNonNull` helper and the
  latter with `?? 0`, both explained inline. Full frontend gate suite and the Phase
  11 live RLS smoke test (`verify-data-layer.mjs`, 8/8) both re-run clean against the
  new types.
...
- [X] Audit every requirement and accepted frontend flow against coverage. Done 2026-07-28, cross-referencing every requirement in `.agents/plan.md` and every journey in `.agents/flow.md` against the evidence trail in this file. Full findings not reproduced here (they'd duplicate this file); summary:
  - **7 plan.md requirements with no implementation evidence anywhere** — concentrated in exactly the backend-integration scope this phase's own scope note already named as deferred (real E2E against Supabase data, staging environments, audit retention policy), plus one independent gap: the Phase 2C `MachineTechnicalProfile` typed-value-plus-unit contract is still frozen but unused, and engineering sign-off on the final machine field set was never obtained.
  - **7 flow.md user journeys with no live coverage** — all trace back to the same Phase 11/12 "data layer built, UI wiring deferred" split: every journey requiring an end-to-end path through the *real, wired-in* app (image upload through the actual uploader, Supervisor cross-department UI rejection, dashboard aggregates against live data, session revocation on password reset) is currently unprovable because nothing calls the real backend from a page yet.
  - **7 "done" items that are honestly self-flagged as thin** — most already carry their own caveat in this file (e.g. Phase 10's account-creation-surface test, Phase 12's MIME-spoof-bypass path), so this is confirmation the self-reporting has been accurate, not a new discovery of hidden gaps. One real finding: several Phase 2–4 verification checkpoints are checked `[x]` on the strength of the user's explicit sign-off rather than a recorded human walkthrough — correctly and repeatedly disclosed in the surrounding text, but a bare glance at the checkbox alone would overstate it.
  - **3 real contradictions between documented decisions and live practice** — (a) `plan.md` specifies separate staging infrastructure; none was ever provisioned, everything ran against the one live project (a scope compression, not a hidden gap — already disclosed above); (b) `disable_signup` was live-`false` (self-registration open) for a period, directly contradicting `plan.md`'s "public self-registration remains disabled" rule, until caught and fixed 2026-07-28; (c) the Phase 9 DB-password-exposure incident remains an open, disclosed exception (rotation deferred to the user).
  - No plan.md decision was found reversed or silently ignored beyond plan.md's own recorded pivots (Viewer role removal, single-image model) — both already correctly reflected here.
...
- [X] Test concurrency, transaction rollback, expired sessions, function timeout, Cloudinary compensation, and cleanup retries. **Partially done — the parts achievable against the live database/Edge Functions directly**, `supabase/scripts/verify-concurrency-and-sessions.mjs`: two simultaneous machine-creation requests sharing a code race against `machines_code_key` and exactly one wins, confirmed by count (not just by one request's own response); an insert violating a constraint (unknown `department_id`) leaves zero partial rows; a malformed token and a well-formed-but-forged/expired-looking token are both rejected the same way. **7/7 checks passing.** Function timeout: tested manually with a disposable Edge Function that slept for a requested duration (deployed, tested, deleted — see `supabase/scripts/README.md`'s "Function-timeout behavior" section) — 5s and 60s completed normally, 170s was killed by the platform partway through (~150s measured) with `WORKER_RESOURCE_LIMIT`; confirms the platform enforces its own limit independent of anything this project's functions do, and none of the three deployed functions do long-running work that could hit it. Cloudinary compensation/cleanup retries specifically as *automated* paths remain undone (the manual `reconcile-cloudinary-orphans.mjs` substitute from Phase 12 is the closest equivalent, not a retry mechanism).
...
- [X] Document local/staging/production envs, migrations, generated types, project-owner Auth bootstrap/offboarding, backups/restore, forward-fix/rollback, audit retention, and Cloudinary reconciliation. `supabase/RUNBOOK.md`, written 2026-07-28, consolidates all of it in one place — and says so plainly where something isn't actually done yet (no staging environment exists; generated types are hand-written, not tool-generated; backups have never been independently confirmed or test-restored; audit retention policy is still an open decision) rather than describing an aspirational process as if it were real. `supabase/README.md` was also rewritten — it was stale since Phase 8 and had drifted into actively claiming "no deployed functions, no connection to a live project," both long since false.
...
- [X] Documentation can be followed from a clean environment. Done 2026-07-28 via a fresh-context subagent walkthrough (no memory of this session, only the docs plus the real repo/live project to check claims against). Found the docs "unusually good" overall, with 3 concrete issues, all fixed the same day:
  1. **The single most valuable finding**: `RUNBOOK.md`'s claim that `gen types` requires Docker was independently re-tested by the walkthrough (it actually ran the command) and found **false** for CLI 2.110.0 — see the `database.types.ts` regeneration above, which this finding directly triggered.
  2. Stale "SMTP is still an open decision" language in `bootstrap-user.mjs`'s header comment and `scripts/README.md`, left over from before SMTP was wired earlier this same day — both corrected.
  3. A disposable `timeout-test` Edge Function (deployed for the function-timeout check above) was still live and untracked in git with no documentation trail — deleted (both from the live project and locally) once its purpose was served.
  No dead ends were found in the bootstrap, Edge Function deploy, or Cloudinary reconciliation instructions — all matched the real files/commands exactly.
