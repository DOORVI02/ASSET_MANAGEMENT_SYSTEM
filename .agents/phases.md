...
- [ ] Connect accepted uploader progress/preview/error/cancel UI. Not started — `ImageUploader` still preview-only, per the UI's own "Preview-only image handling" banner. Backend (sign/upload/finalize/delete) is now fully ready for this wiring.
...
- [ ] Compensate or record cleanup pending when finalization fails. Not implemented as an automated retry/reconciliation path — if `cloudinary-finalize` fails after a real Cloudinary upload succeeded, the asset is orphaned in Cloudinary until manually or reconciliation-job cleaned up. The `attachments.status` enum (`pending`/`ready`/`deleting`/`failed`) exists in the schema for this purpose but nothing sets it to anything but `ready` yet.
...
- [X] Reconcile sandbox Cloudinary assets with attachment rows after upload/replace/delete. Exercised manually via direct Cloudinary Admin API resources listings after both verify scripts' full runs — zero leftover assets both times. Not a standing/automated reconciliation job (see the orphan-reconciliation gap above), but the manual check itself now has real evidence behind it across the full lifecycle, not just a single upload.
