/**
 * Real `supabase gen types typescript --linked` output — regenerated 2026-07-29.
 *
 ...
 *
 * Note when reading view types: Postgres reports every column of a view as nullable,
 * even where the underlying join is through a NOT NULL foreign key that can never
 * produce one. `mappers.ts` handles that with a documented `assumeNonNull` helper for
 * base columns and `?? 0` for the aggregate-count views — re-check both after each
 * regeneration, in case a migration genuinely changed what can be null.
 */
export type Json =
  | string
  | number
